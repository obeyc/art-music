<?php
    ...
    if ($search_query === "No Search Term!") {
        ...
        $query = "SELECT * FROM videos WHERE TheatreID=" . $user_theatre['id'] . " ORDER BY id DESC LIMIT 50";
    ...
    if (isset($_POST['addSubmit'])) {
                ...
                try {
                   ...
                   // print_r($response);
                    if ($response['body']['transcode']['status'] === 'complete') {
                        $success_message = 'Your video finished uploading.';
                    } elseif ($response['body']['transcode']['status'] === 'in_progress') {
                        $progress_message = 'Your video is still uploading.';
                    } else {
                        $error_message = 'Your video encountered an error during uploading.';
                    }

                    $response = $client->request($uri . '?fields=link');
                    //$vimeoLink = "https://player.vimeo.com" . str_replace('s', '', $uri);
                    $vimeoLink = $response['body']['link'];
                    //print_r($response);
                } catch(Exception $e) {
        ...
        if (empty($error_message)) {
            ...
            $addshortdesc = htmlentities($shortDescription,ENT_QUOTES);
            $addlongdesc = htmlentities($longDescription,ENT_QUOTES);
            $addtitle = htmlentities($title,ENT_QUOTES);
            ...
            $stmt->bind_param("sssssiissssi", $addtitle, $addshortdesc, $addlongdesc, $length, $price, $user_theatre['id'], $category, $starring, $releaseDate, $vimeoLink, $videoType, $default_popularity);
            $stmt->execute();
    ...
    if (isset($_POST['updateSubmit'])) {
        ...
        if (isset($_POST['updateShortDescription']) && !empty($_POST['updateShortDescription'])) {
            $shortDescription = $_POST['updateShortDescription'];
        } else {
            $error_message = "Please provide a Short Description.";
        }
        ...
        if (empty($error_message)) {
            $db = new mysqli($servname, $username, $password, $database);
            $updateshortdesc = htmlentities($shortDescription,ENT_QUOTES);
            $updatelongdesc = htmlentities($longDescription,ENT_QUOTES);
            ...
            $stmt->bind_param("ssssissssi", $updateshortdesc, $updatelongdesc, $length, $price, $category, $starring, $releaseDate, $vimeoLink, $videoType, $selected_video['id']);
            $stmt->execute();
                                    ...
                                    <div class="col-md-2">
                                        <h6 class="mobile-text-left">
                                            <?php
                                                ...
                                                echo '<i class="bi-eye text-success"></i> '.$video_views->num_rows;
                                            ?>
                                            views
                                        </h6>
                                    </div>
                                    <div class="col-md-2">
                                        <h6 class="mobile-text-left mobile-margin-b-2"><?php echo date('d M Y H:i',strtotime($video['ReleaseDate'])); ?></h6>
                                    </div>
                                    <div class="col-md-2">
