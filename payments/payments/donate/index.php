<?php
    ...
    if (isset($_POST['submit'])) {
        if (isset($_POST['amount']) && !empty($_POST['amount'])) {
            ...
            $error_message = "Please provide an amount to donate.";
        }
        ...
        if (empty($error_message)) {
            header('Location: ./checkout.php?vid=' . $selected_video . '&a=' . $amount);
        }
    }
...
<head>
    <meta charset="UTF-8">
    <meta name="description" content="Make a donation toward you favoutite videos!">
    <meta name="keywords" content="artmusic, donate">
    <meta name="author" content="Dominic Mostert">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ArtMusic TV - Donate</title>
                ...
                <div class="jumbotron py-4 mb-5">
                    ...
                    <form action="" method="POST">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="amount" name="amount" value="<?php echo $amount; ?>" min="50" required>
                        </div>
                        <div class="form-group">
                            <select class="form-control" id="video" name="video" required>
                                <option value="">Select a Video</option>
                                <?php while ($video = $all_videos->fetch_assoc()) { ?>
                                    <option value="<?php echo $video['id']; ?>" <?php if (isset($selected_video) && $selected_video == $video['id']) { echo "selected"; } ?>><?php echo $video['Title']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <button type="submit" class="btn btn-success" name="submit">Make Donation</button>
                    </form>
                </div>
            </div>
        </div>
