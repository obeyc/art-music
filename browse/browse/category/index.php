<?php
    ...
    if (isset($_GET['cid']) && !empty($_GET['cid'])) {
        if (is_numeric($_GET['cid'])) {
            ...
            $current_category = $db->query($query);

            if ($current_category->num_rows > 0) {
                $current_category = $current_category->fetch_assoc();
            } else {
                $category_id = 0;
            }
        } else {
    ...
    $all_categories = $db->query($query);

    /* Pagination */

    if ($category_id === 0) {
        $query = "SELECT * FROM videos WHERE CategoryID<>0";
    } else {
    ...
    if ($category_id === 0) {
        $query = "SELECT * FROM videos WHERE CategoryID<>0 ORDER BY ReleaseDate DESC LIMIT $start, 12";
    } else {
        $query = "SELECT * FROM videos WHERE CategoryID=$category_id ORDER BY ReleaseDate DESC LIMIT $start, 12";
    }
    $category_videos = $db->query($query);
...
<head>
<meta charset="UTF-8">
    <meta name="description" content="Browse the video categories on ArtMusic TV">
    <meta name="keywords" content="artmusic tv, categories">
    <meta name="author" content="Dominic Mostert">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ArtMusic TV - Categories</title>
                ...
                <div class="mb-5">
                    <?php while($category = $all_categories->fetch_assoc()) : ?>
                        <a href="./?cid=<?php echo $category['id']; ?>">
                            <span class="badge badge-primary p-2"><?php echo $category['Title']; ?></span>
                        </a>
                    <?php endwhile; ?>
                </div>

                <?php if ($page_total == 0) { ?>
                    <h1 class="text-center my-5">There are no videos in this categories at the moment.</h1>
                <?php } else { ?>
                <!-- Categories Videos Section -->
                <section class="mb-5">
                    <?php while($video = $category_videos->fetch_assoc()) : ?>
                        <?php
        ...
        <div class="mb-5">
            <ul class="pagination justify-content-center">
                <li class="page-item <?php if ($page - 1 < 1) { echo "disabled"; } ?>">
                    <a class="page-link" href="./?cid=<?php echo $category_id; ?>&p=<?php echo $page - 1; ?>">Prev</a>
                </li>
                <li class="px-4 pt-1">
                ...
                <li class="page-item <?php if ($page + 1 > $page_total) { echo "disabled"; } ?>">
                    <a class="page-link" href="./?cid=<?php echo $category_id; ?>&p=<?php echo $page + 1; ?>">Next</a>
                </li>
            </ul>
        </div>
    </main>
