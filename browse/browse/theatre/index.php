<?php
    ...
    if (isset($_GET['tid']) && !empty($_GET['tid'])) {
        if (is_numeric($_GET['tid'])) {
            ...
            $current_theatre = $db->query($query);

            if ($current_theatre->num_rows > 0) {
                $current_theatre = $current_theatre->fetch_assoc();
            } else {
                $theatre_id = 0;
            }
        } else {
    ...
    if ($theatre_id === 0) {
        $query = "SELECT * FROM videos WHERE TheatreID<>0";
    } else {
        $query = "SELECT * FROM videos WHERE TheatreID=$theatre_id";
    }

    $result = $db->query($query);
    
    $page_total = ceil($result->num_rows/12);
    ...
    if ($theatre_id === 0) {
        $query = "SELECT * FROM videos WHERE TheatreID<>0 ORDER BY ReleaseDate DESC LIMIT $start, 12";
    } else {
        $query = "SELECT * FROM videos WHERE TheatreID=$theatre_id ORDER BY ReleaseDate DESC LIMIT $start, 12";
    }
...
<head>
<meta charset="UTF-8">
    <meta name="description" content="Browse the theatres on ArtMusic TV">
    <meta name="keywords" content="artmusic tv, theatres">
    <meta name="author" content="Dominic Mostert">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ArtMusic TV - Theatres</title>
    ...
    <main class="mx-4">
        ...
        <div class="row">
            <div class="col-md-10 mx-auto">
                ...
                <div class="mb-5">
                    <?php while($theatre = $all_theatres->fetch_assoc()) : ?>
                        <a href="./?tid=<?php echo $theatre['id']; ?>">
                            <span class="badge badge-primary p-2 rrv_badge-title"><?php echo $theatre['Title']; ?></span>
                        </a>
                    <?php endwhile; ?>
                </div>

                <?php if ($page_total == 0) { ?>
                    <h1 class="text-center my-5">There are no videos in this theatres at the moment.</h1>
                <?php } else { ?>
                <!-- Theatres Videos Section -->
                <section class="mb-5">
                    <?php while($video = $theatre_videos->fetch_assoc()) : ?>
                        <?php
        ...
        <div class="mb-5">
            <ul class="pagination justify-content-center">
                <li class="page-item <?php if ($page - 1 < 1) { echo "disabled"; } ?>">
                    <a class="page-link" href="./?tid=<?php echo $theatre_id; ?>&p=<?php echo $page - 1; ?>">Prev</a>
                </li>
                <li class="px-4 pt-1">
                ...
                <li class="page-item <?php if ($page + 1 > $page_total) { echo "disabled"; } ?>">
                    <a class="page-link" href="./?tid=<?php echo $theatre_id; ?>&p=<?php echo $page + 1; ?>">Next</a>
                </li>
            </ul>
        </div>
    </main>
